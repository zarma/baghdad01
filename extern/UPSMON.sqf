...
if (KRON_UPS_Debug>0) then {player sidechat format["%1: New instance %2 %3 %4",_grpidx,_npc getVariable ("UPSMON_grpid"),_npc,typeof _npc]}; 
...
// create the clones
	for "_grpcnt" from 1 to _copies do {
		...
		_nul=[_lead,_areamarker,_pause,_noslow,_nomove,_nofollow,_initpos,_track,_showmarker,_shareinfo,"DELETE:",_deletedead] execVM "extern\upsmon.sqf";
		//sleep .05;
	};	
	processInitCommands;
	sleep .05;
	...
	if ((_issoldier) && !(_exit)) then {
		...
		//Reveal targets found by members to leader
		{
			...
			if (_x knowsabout _NearestEnemy > 0.5 && (_npc knowsabout _NearestEnemy <= 0.5 || count _targets <= 0 )) then 	{		
				...
				if (count _targets <= 0) then {
					...
					_target setvariable ["UPSMON_lastknownpos", position _target, false];						
		...
		if ((_shareinfo=="SHARE")) then {			
			...
			if ( !isNull (_newtarget) && alive _newtarget && canmove _newtarget && (_newtarget != _target || isNull (_target)) ) then {
				...
				_target = _newtarget;		
				//if (KRON_UPS_Debug>0) then {player globalchat format["%1: TARGET %2 knows=%3",_grpidx, typeof _target, _npc knowsabout _target]}; 	
			};						
		};		
		
		//Enemy detected
		if (_fightmode != "fight" && count _targets > 0) then {
			_fightmode = "fight";
		...
		if ( _fightmode != "walk" && !isnull(_target) && _dist < 25 &&  _npc knowsabout _target < 0.5 ) then {			
		...
		//if (KRON_UPS_Debug>0) then {player sidechat format["%1: _nowp=%2 in vehicle=%3 _inheli=%4 _fightmode=%5",_grpidx,_nowp,vehicle (_npc) != _npc,_inheli,_fightmode ]}; 	
		//If in vehicle take driver if not controlled by user
		if (alive _npc && !_nowp) then {
			if (vehicle (_npc) != _npc && !_inboat && !(vehicle (_npc) iskindof "StaticWeapon")) then { 						
				...
				if (!_inheli) then { 						
					if (_fightmode == "walk") then {
						...
						_GetOutDist =  _closeenough  * ((random .5) + 0.5);
					};
...
//**********************************************************************************************************************		
//if (KRON_UPS_Debug>0) then {player sidechat format["%1:dist=%2 pursue=%3 %7 timeon=%4 enemies=%5 %6",_grpidx,_dist,_pursue,_timeontarget,count _enemies, _Behaviour,_fightmode]}; 					
		if (_pursue) then {			
			_pursue = false;
			_newpos = true; 	
			_react = 0;		
...
//**********************************************************************************************************************
	if !(_newpos) then {
		...
		//Stuck control
		if (!_nowp && alive _npc && canmove _npc && _wptype == "MOVE" && _timeontarget >= 120 && _lastcurrpos select 0 == _currpos select 0 && _lastcurrpos select 1 == _currpos select 1) then {
		...
		if (_waiting<0) then {
			...
			if (!_nowp && (!_gothit) && (!_swimming) && (_fightmode == "walk") && (( _targetdist <= (_area/5) || moveToFailed _npc) && (_timeontarget > KRON_UPS_maxwaiting))) then {
				_makenewtarget=true;
				_unitpos = "AUTO";
				_Behaviour = _orgMode;
			};
		...
		if (_gothit && _npc == vehicle (_npc) && alive _npc ) then {																
			...
			if ((random 100)<50) then {				
				...
				//Cuerpo a tierra
				{
					...
					if ( _x == vehicle _x && canmove _x && alive _x) then {																		
		...
		if (alive _npc) then {
			...
			//Si el waypoing es distinto del que tiene o es diferente de hold lo establecemos
				IF (_wptype != "HOLD" || _lastwptype != _wptype) then {														
					...
					if ( !isnull _grp && !isnil "_targetPos") then {		
			...
			{
				if ((random 100)<95 && _x == vehicle _x && !((primaryWeapon _x ) in KRON_UPS_MG_WEAPONS)) then {
			...
			if (_gothit || _dist <= _closeenough) then {
				{
					if (!canStand _x || ((primaryWeapon _x ) in KRON_UPS_MG_WEAPONS) || (vehicle _x == _x && (random 100) < 50) ) then {						
						_x suppressFor 20;
					};
				} foreach units _npc;									
			};							
